 ...
 */(function(Highcharts){var UNDEFINED,win=Highcharts.win,DIV="div",ABSOLUTE="absolute",RELATIVE="relative",HIDDEN="hidden",VISIBLE="visible",PX="px",css=Highcharts.css,CanVGRenderer=Highcharts.CanVGRenderer,SVGRenderer=Highcharts.SVGRenderer,extend=Highcharts.extend,merge=Highcharts.merge,addEvent=Highcharts.addEvent,createElement=Highcharts.createElement,discardElement=Highcharts.discardElement;extend(CanVGRenderer.prototype,SVGRenderer.prototype);extend(CanVGRenderer.prototype,{create:function(chart,container,chartWidth,chartHeight){this.setContainer(container,chartWidth,chartHeight);this.configure(chart)},setContainer:function(container,chartWidth,chartHeight){var containerStyle=container.style,containerParent=container.parentNode,containerLeft=containerStyle.left,containerTop=containerStyle.top,containerOffsetWidth=container.offsetWidth,containerOffsetHeight=container.offsetHeight,canvas,initialHiddenStyle={visibility:HIDDEN,position:ABSOLUTE};this.init(container,chartWidth,chartHeight);canvas=createElement("canvas",{width:containerOffsetWidth,height:containerOffsetHeight},{position:RELATIVE,left:containerLeft,top:containerTop},container);this.canvas=canvas;this.ttLine=createElement(DIV,null,initialHiddenStyle,containerParent);this.ttDiv=createElement(DIV,null,initialHiddenStyle,containerParent);this.ttTimer=UNDEFINED;var hiddenSvg=createElement(DIV,{width:containerOffsetWidth,height:containerOffsetHeight},{visibility:HIDDEN,left:containerLeft,top:containerTop},containerParent);this.hiddenSvg=hiddenSvg;hiddenSvg.appendChild(this.box)},configure:function(chart){var renderer=this,options=chart.options.tooltip,borderWidth=options.borderWidth,tooltipDiv=renderer.ttDiv,tooltipDivStyle=options.style,tooltipLine=renderer.ttLine,padding=parseInt(tooltipDivStyle.padding,10);tooltipDivStyle=merge(tooltipDivStyle,{padding:padding+PX,"background-color":options.backgroundColor,"border-style":"solid","border-width":borderWidth+PX,"border-radius":options.borderRadius+PX});if(options.shadow){tooltipDivStyle=merge(tooltipDivStyle,{"box-shadow":"1px 1px 3px gray","-webkit-box-shadow":"1px 1px 3px gray"})}css(tooltipDiv,tooltipDivStyle);css(tooltipLine,{"border-left":"1px solid darkgray"});addEvent(chart,"tooltipRefresh",(function(args){var chartContainer=chart.container,offsetLeft=chartContainer.offsetLeft,offsetTop=chartContainer.offsetTop,position;tooltipDiv.innerHTML=args.text;position=chart.tooltip.getPosition(tooltipDiv.offsetWidth,tooltipDiv.offsetHeight,{plotX:args.x,plotY:args.y});css(tooltipDiv,{visibility:VISIBLE,left:position.x+PX,top:position.y+PX,"border-color":args.borderColor});css(tooltipLine,{visibility:VISIBLE,left:offsetLeft+args.x+PX,top:offsetTop+chart.plotTop+PX,height:chart.plotHeight+PX});if(renderer.ttTimer!==UNDEFINED){clearTimeout(renderer.ttTimer)}renderer.ttTimer=setTimeout((function(){css(tooltipDiv,{visibility:HIDDEN});css(tooltipLine,{visibility:HIDDEN})}),3e3)}))},destroy:function(){var renderer=this;discardElement(renderer.canvas);if(renderer.ttTimer!==UNDEFINED){clearTimeout(renderer.ttTimer)}discardElement(renderer.ttLine);discardElement(renderer.ttDiv);discardElement(renderer.hiddenSvg);return SVGRenderer.prototype.destroy.apply(renderer)},color:function(color,elem,prop){if(color&&color.linearGradient){color=color.stops[color.stops.length-1][1]}return SVGRenderer.prototype.color.call(this,color,elem,prop)},draw:function(){var renderer=this;win.canvg(renderer.canvas,renderer.hiddenSvg.innerHTML)}})})(Highcharts);