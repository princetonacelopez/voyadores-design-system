$(function () {
    ...
    $("body").on('mouseenter', '[data-toggle="tooltip"]', function () {
        ...
        // Checks if the element has a class of `text-truncate` and if it is a table cell
        if (element.hasClass("text-truncate") || element.is('td')) {

            if (element.is('td')) {
                const $table = element.closest('table');

                // Ensure a table ancestor exists, it actually contains the td, and it has class 'table'
                if (!$table.length || !$table[0].contains(element[0]) || !$table.hasClass('table-custom')) {
                    return;
                }
            }
