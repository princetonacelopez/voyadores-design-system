...
import UserNotificationGetViewModel from "./application.notifications.get.js";
...
ulNotificationMenu.repeater({
    ...
    messageNoResultHeader   : 'You’re all caught up',
    messageNoResult         : `You don’t have any notifications`,
    cssClassNoResult        : 'text-center status-text flex-grow-1 bg-transparent border-0',
...
if (!isNotificationPage) {
    ...
    setInterval(notificationInterval, minInterval);
}

async function notificationInterval() {
    notifications.length = 0; // resets to empty

    const response = await $.callAsync(getViewModel.getTopNotifications);

    if (!response || !Array.isArray(response.TopNotifications)) return;

    const unreadCount = response.UnreadCount;

    notifications.push(...response.TopNotifications);
...
function buildNotificationMenuListHTML(notifications) {
    ...
    for (const notification of notifications) {
        ...
        buildAvatarContainer(notification.Id, metadata.PageIcon, metadata.ImageFilename);

        const relativeTime = moment(notification.CreatedAt).from(moment());

        html += `<li class="nav-item">
                    <a data-id="${notification.Id}" data-status="${notification.State}" data-redirect-to="${metadata.RedirectPath}" class="btn-read nav-link fs-3 fs-lg-5 p-4 d-flex gap-4 rounded ${notification.State == InAppState.Unread ? 'notification-unread' : ''}" href="javascript:void(0)">
...
function buildAvatarContainer(id, pageIcon, filename) {
    if (!filename) return;

    const imgHtml = `
        <div class="avatar-xl d-grid place-items-center flex-shrink-0 position-relative">
            <img src="/files/get-file?fileName=${filename}" class="rounded-circle border object-fit-cover" width="47.99" height="47.99">
            <span class="position-absolute bottom-0 end-0 p-2 bg-primary rounded-circle">
    ...
    img.src = `/files/get-file?fileName=${filename}`;
